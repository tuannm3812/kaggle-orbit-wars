...
account `tuannm3812` has `roi_reserve_v5` and `roi_reserve_v6` challenger
submissions under public evaluation.
...
| --- | --- | --- | --- | --- |
| `roi_reserve_v6_submission.tar.gz` | `2026-06-04 21:39:17.503000` | `roi_reserve_v6 bounded regroup challenger` | `SubmissionStatus.COMPLETE` | `506.8` |
| `submission.tar.gz` | `2026-06-04 12:53:18.993000` | `roi_reserve_v5 regroup pass from producer-style pressure analysis` | `SubmissionStatus.COMPLETE` | `467.6` |
...
| `roi_reserve_v5` | `notebooks/02_agent_submission.ipynb` / `kaggle/submission/02_agent_submission.ipynb` / `agents/roi_reserve_v5/main.py` | Yes, on `tuannm3812`; submission ids `53353907` and `53363032` | `467.6` | Adds replay-driven combat survival: source garrison projection after launch, aggregate incoming-fleet safety, visible enemy support pressure, multiplayer enemy-target restraint, and then a bounded Producer-style regroup pass. Kaggle-hosted smoke benchmark: 30 wins, 0 losses, 0 errors over 30 seeds vs `random`. | Regroup pass recovered some score from `443.5` to `467.6`, but the family still sits below v4 and v3. Keep as archive and use the loss pattern to design v6. |
| `roi_reserve_v6` | `notebooks/02_agent_submission.ipynb` / `kaggle/submission/02_agent_submission.ipynb` / `agents/roi_reserve_v6/main.py` | Yes, on `tuannm3812`; submission id `53374152` | `506.8` | Keeps the v3 opening shape and adds a bounded regroup pass for surplus ships in pressure hotspots, plus a milder source-safety gate than v5. | Better than v5 and within range of v3, but still slightly below the stable baseline. Keep as the active challenger while replay review decides whether regroup or source safety needs the next change. |
