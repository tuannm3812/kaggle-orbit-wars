...
| --- | --- | --- | --- | --- |
| `submission.tar.gz` | `2026-06-03 09:41:07.643000` | `roi_reserve_v2: orbit-aware aiming, target reservation, stronger reserves` | `SubmissionStatus.COMPLETE` | `446.5` |
...
| `roi_reserve_v2` | `kaggle/roi_reserve_v2/03_roi_reserve_agent_v2.ipynb` / `agents/roi_reserve_v2/main.py` | Yes | `446.5` | Adds orbit-aware aiming, per-turn target reservation, and stronger early reserve. Kaggle smoke benchmark: 27 wins, 3 losses, 0 errors over 30 seeds vs `random`. Submitted from notebook output `submission.tar.gz`. | Current champion, but public replays show opening tempo, long-travel target selection, and missing defense are the main v3 targets. |
