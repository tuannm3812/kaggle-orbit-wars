...
| --- | --- | --- | --- | --- |
| `submission.tar.gz` | `2026-06-04 07:29:16` | `roi_reserve_v5: combat-survival source safety and contested-target pressure` | `SubmissionStatus.COMPLETE` | `518.4` |
...
| `roi_reserve_v5` | `notebooks/02_agent_submission.ipynb` / `kaggle/submission/02_agent_submission.ipynb` / `agents/roi_reserve_v5/main.py` | Yes, on `tuannm3812`; submission id `53353907` | `518.4` | Adds replay-driven combat survival: source garrison projection after launch, aggregate incoming-fleet safety, visible enemy support pressure, and multiplayer enemy-target restraint. Kaggle-hosted smoke benchmark: 30 wins, 0 losses, 0 errors over 30 seeds vs `random`. | Volatile early challenger, currently slightly above v3 but not proven. First v5 replays include one win and two elimination losses; collect more replays before promotion. |
